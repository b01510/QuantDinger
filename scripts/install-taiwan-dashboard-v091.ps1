...
if (-not $status -or -not $status.StartsWith('Up')) { docker logs $Container --tail 80; throw 'QuantDinger Taiwan v0.9.1 啟動失敗。' }
Write-Host 'QuantDinger Taiwan v0.9.1 更新完成' -ForegroundColor Green
Write-Host '日K/均線新鮮度判斷已修正，SQLite 快取保留並會自動補到最新交易日。'
